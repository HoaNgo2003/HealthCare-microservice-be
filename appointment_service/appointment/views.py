...
class AppointmentView(APIView):
    ...
    def get_patient_id(self, request):
        ...
        try:
            resp = requests.get(AUTH_SERVICE_USERINFO_URL, headers={'Authorization': auth_header})
            if resp.status_code != 200:
                return None, Response({'detail': 'Failed to get user info from AuthService'}, status=resp.status_code)
            user_data = resp.json()
            if user_data.get('role') != 'patient':
                return None, Response({'detail': 'User is not patient'}, status=status.HTTP_403_FORBIDDEN)
            return user_data.get('id'), None
        except Exception as e:
            return None, Response({'detail': 'Error contacting AuthService', 'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def get(self, request):
        ...
        appointments = Appointment.objects.filter(patient_id=patient_id).order_by('-appointment_time')
    ...
    def post(self, request):
        ...
        if not doctor_id or not appointment_time:
            return Response({'detail': 'doctor_id and appointment_time are required'}, status=status.HTTP_400_BAD_REQUEST)
...
class GetListAppointmentByDoctorId(APIView):
    ...
    def get(self, request, doctor_id):
        appointments = Appointment.objects.filter(doctor_id=doctor_id).order_by('-appointment_time')
        serializer = AppointmentSerializer(appointments, many=True)
        return Response(serializer.data)
